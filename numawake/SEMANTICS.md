...
本文档明确 numawake 工具输出的两类 NUMA 相关事件及其判据，供 BPF 实现与用户态聚合共用。实现细节见 [VERIFY_SYMBOL.md](VERIFY_SYMBOL.md)。
...
- 名称保留「wake」以强调事件锚定在 wake 路径，而非周期性 load balance（后者需过滤 `select_task_rq_fair` 非 wake 调用，见 VERIFY_SYMBOL.md）。
